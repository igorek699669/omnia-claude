...
2. **Better Auth, телефон + OTP** — ✅ подключено (`auth.ts`, плагин `phoneNumber`, SMS через SMS.ru). Раньше был вход по email-коду — заменён целиком, `emailOTP` больше не используется. `orders.customerId` ссылается на Better Auth user; админы — в родной auth Payload, не смешивать.
...
Без `DATABASE_URL` сайт работает, но `/auth` не сможет отправить код (Better Auth падает на подключении к БД). Без `SMS_RU_API_ID` вход работает: код печатается в консоль сервера. `BETTER_AUTH_SECRET` уже сгенерирован в `.env.local` — трогать не нужно.
