...
/**
 * Лимит на запуск проверки номера. Каждый startPhoneCall заводит платную проверку в SMS.ru,
 * а server action открыт всему интернету: скрипт в цикле выносит баланс за минуты, а вместе
 * с ним ложатся вход на /auth и подтверждение телефона на чекауте — то есть все продажи.
 *
 * Счётчик в памяти процесса, а не в Postgres: приложение крутится одним контейнером, делить
 * состояние не с кем, а новая таблица означала бы миграцию схемы Payload. Плата — счётчики
 * обнуляются при перезапуске, но он редок и атакующему неподконтролен.
 */
...
/**
 * Проверяет оба лимита и, если оба пройдены, засчитывает попытку. Проверка и запись
 * разнесены: иначе, упершись в лимит по IP, можно было бы заодно исчерпать чужой номер.
