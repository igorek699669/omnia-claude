 ...
 *
 * Почему не SMS: отправка сообщений требует регистрации буквенного отправителя у операторов,
 * без неё они молча отклоняют доставку (SMS.ru принимает сообщение, а статус приходит 107
 * «не может быть доставлено» и деньги возвращаются). Flash call — звонок пользователю, где
 * код это последние цифры номера, — операторы с 2023 года режут как спам, и SMS.ru свой
 * метод code/call уже пометил устаревшим. Остаётся обратный сценарий: номер называем мы,
 * звонит покупатель. Звонок бесплатный для него (номер 8-800), SMS.ru его сбрасывает.
 */
// Переопределяется только в E2E (e2e/mocks): настоящая проверка номера платная и требует
// живого входящего звонка, в пайплайне её изображает локальная заглушка.
...
/**
 * Регистрирует проверку номера и возвращает телефон, на который покупатель должен позвонить.
 * У него есть 5 минут; звонок SMS.ru сбрасывает, так что для него он бесплатный.
 */
